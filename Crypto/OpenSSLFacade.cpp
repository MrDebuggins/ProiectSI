...
using namespace OpenSSLFacade;


void OpenSSLFacade::genSymmetricKey(void* cipher, std::string filePath)
{
...
void OpenSSLFacade::readSymmetricKey(std::string filePath, char* key, char* iv)
{
	char line[128];
...
float OpenSSLFacade::symmetricEncrypt(std::string keyFilePath, std::string dataFilePath, void* cipher)
{
...
float OpenSSLFacade::symmetricDecrypt(std::string keyFilePath, std::string dataFilePath, void* cipher)
{
	std::string tempOutPath = dataFilePath + "tmp";							// temporary output file path
	std::ifstream keyFile(keyFilePath);										// stream to check if key file exists
	std::ifstream input(dataFilePath, std::ios_base::binary);			// input data file stream
...
void OpenSSLFacade::genRSAKey(int size, std::string filePath)
{
...
void OpenSSLFacade::saveSeal(std::string filePath, unsigned char* key, int keyLen, unsigned char* iv)
{
	BIO* b64 = BIO_new(BIO_f_base64());
...
void OpenSSLFacade::readSeal(std::string filePath, unsigned char* key, int* keyLen, unsigned char* iv)
{
	char line[1024];
...
EVP_PKEY* OpenSSLFacade::readRSAPublicKey(std::string filePath)
{
	EVP_PKEY* key = nullptr;
...
EVP_PKEY* OpenSSLFacade::readRSAPrivateKey(std::string filePath)
{
	EVP_PKEY* key = nullptr;
...
float OpenSSLFacade::asymmetricEncrypt(std::string keyFilePath, std::string dataFilePath)
{
...
float OpenSSLFacade::asymmetricDecrypt(std::string keyFilePath, std::string dataFilePath)
{
	std::string tempOutPath = dataFilePath + "tmp";												// output stream file path
	std::ifstream input(dataFilePath, std::ios_base::binary);								// input stream
	std::ofstream output(tempOutPath, std::ios_base::binary);								// output stream
...
std::string OpenSSLFacade::hash(std::string filePath)
{
